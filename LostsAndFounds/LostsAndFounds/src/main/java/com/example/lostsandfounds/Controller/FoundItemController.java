...
public class FoundItemController {
    ...
    public ResponseEntity addItem(@Valid @RequestBody FoundItem item, Errors e) {
        ...
        }else return ResponseEntity.status(400).body(new ApiResponse("The user or the lost item are not found"));
    ...
    public ResponseEntity deleteItem(@PathVariable Integer id) {
        ...
        if (foundItemService.deleteFound(id)) {
            return ResponseEntity.status(200).body(new ApiResponse("The item is deleted successfully"));

        }
        return ResponseEntity.status(400).body(new ApiResponse("The  item are not found"));
